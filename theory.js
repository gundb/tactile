	...
	function theorize(a){
		...
		a.obj = (function(){
			...
			obj.each = (function(l,c,_){
				...
				var u, i = 0, ii = 0, x, r, rr, ll, f = a.fns.is(c),
				t = (function(k,v){
				...
				if(a.list.is(l) || ll){
					...
					for(;i < x; i++){
						ii = (i + a.list.index);
						if(f){
							r = ll? c.call(_ || this, l[ll[i]], ll[i], t) : c.call(_ || this, l[i], ii, t);
							if(r !== u){ return r }
						} else {
							if(a.test.is(c,l[ll? ll[i] : i])){ return ll? ll[i] : ii }
						}
					}
				} else if(a.obj.is(l)){
					for(i in l){
		...
		a.text = (function(){
			...
			text.random = text.r = (function(l,c){
				...
				c = a.text.is($)? $ : a.text.is(c)? c : '0123456789ABCDEFGHIJKLMNOPQRSTUVWXZabcdefghijklmnopqrstuvwxyz';
		...
		a.num = (function(){
			...
			num.is = (function(n){
				n = (($=a.fns.$(num))!==_)?$:n;
				return !a.list.is(n) && (Infinity === n || n - parseFloat(n) + 1 >= 0); // jquery doesn't check for Infinity.
			});
	...
	root.pollute = ((typeof GLOBAL !== 'undefined' && GLOBAL.global && GLOBAL.process &&
		...
		(function(){
			...
			module.exports=(function(cb,deps,name){
				...
				args.file = args.file = util.resolve(args.file, args.file); // fixes some deeply nested issues.
